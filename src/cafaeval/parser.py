        ...
        for line in f:
            ...
            if line and len(line) > 1:
                ...
                if k == "id":
                    ...
                    k == "relationship"
                    and v.startswith("part_of")
                    and "part_of" in valid_rel
                ):
                    s = v.split()[1].strip()
                    rel.append(s)
...
def gt_parser(gt_file, ontologies):
    ...
    for ns in tqdm(ontologies, desc="gt_parser"):
        if gt_dict.get(ns):
            ...
            logging.info(
                "Ground truth: {}, proteins {}, annotations {}, replaced alt. ids {}".format(
...
def pred_parser(pred_file, ontologies, gts, prop_mode, max_terms=None):
            ...
            if line and len(line) > 2:
                ...
                if ns in gts and p_id in gts[ns].ids:
                    ...
                    for term_id in term_ids:
                        if (
                            max_terms is None
                            or np.count_nonzero(matrix[ns][i]) <= max_terms
                        ):
    ...
    for ns in tqdm(ids, desc="pred_parser"):
        if ids[ns]:
            ...
            logging.info(
                "Prediction: {}, {}, proteins {}, annotations {}, replaced alt. ids {}".format(
                    pred_file,
                    ns,
                    len(ids[ns]),
